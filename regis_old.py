...
with tab1:
    ...
    with st.form(key='registration_form', clear_on_submit=True):
        fname = st.text_input(label='First name:', value=fname, placeholder='Enter your first name')
        lname = st.text_input(label='Last name', value=lname, placeholder='Enter your last name:')
        phone = st.text_input(label='Phone number', value=phone, placeholder='Enter your phone number:')
        email = st.text_input(label='Email', value=email, placeholder='Enter your email:')
        position = st.selectbox('Position', position_list, index=position_idx)
        food_allergy = st.checkbox("Food Allergy", value=is_allergy)
        text_food_allergy = st.text_input(label='Allergy', value=text_food_allergy, placeholder='Enter your allergy:')
